...
export function FooterPhysics({
  ...
  useEffect(() => {
    console.log("Board Textures:", boardTextureURLs);
  }, [boardTextureURLs])
  useEffect(() => {
    const handleResize = () => {
  ...
  useEffect(() => {
    ...
    if (!currentScene) {
      console.error("Scene reference is null. Component might not be mounted correctly."); // Debugging check
      return;
    }
    const observer = new IntersectionObserver(
      ([entry]) => {
        console.log("Observer entry:", entry.isIntersecting); // Debugging check
  ...
  useEffect(() => {
    if (!scene.current) {
      console.error("Scene is not defined."); // Debugging check
      return;
    }
    if (!inView) {
      console.warn("Component is not in view, skipping rendering."); // Debugging check
      return;
    }
    ...
    if (prefersReducedMotion)
      console.warn("Skipping animation due to reduced motion settings."); // Debugging check

    console.log("Initializing Matter.js physics..."); // Debugging check
    ...
    return () => {
      ...
      Runner.stop(runner);
      if (currentEngine) {
        World.clear(currentEngine.world, true); // Changed 'false' to 'true' to fully clear objects
